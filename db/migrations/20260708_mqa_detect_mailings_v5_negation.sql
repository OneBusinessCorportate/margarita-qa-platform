-- ---------------------------------------------------------------------------
-- v5: NEGATION handling + broader completion verbs for the keyword detector.
--
-- Bug: «Зарплата — не получил» matched the `получ` stem in the salary/done
-- rule and was stored as «Получил» (completed) — the exact opposite of reality.
-- Any «не отправлено / не сделано / не оплачен» had the same problem.
--
-- Fix (mirrors src/lib/mailings-detect.ts):
--   • A negated completion («не <опц. 1-2 слова> получил/отправил/сделал/…»)
--     no longer counts as done/paid. Done/paid CASEs now require the negation
--     pattern to be ABSENT.
--   • A new `neg` signal is emitted for a negated completion, which derives a
--     "not completed" status instead of leaving the cell neutral:
--       main_taxes/neg      → «Не отправил»
--       salary/primary neg  → «Запросил 1, не получил»
--   • done verbs broadened: + сдела/сделан, готов, выполн, оформ, провед so
--     «Зарплата — сделано» / «Первичка готова» are recognized.
--
-- A real completion later in the same cycle still wins (done_n >= 1 is checked
-- first). Cycle bounds / rollover logic unchanged from the v4 migration.
-- Armenian detection remains handled by the JS runner (src/lib/mailings-run.ts),
-- as in v4 — this SQL cron function is Russian-only.
-- ---------------------------------------------------------------------------
