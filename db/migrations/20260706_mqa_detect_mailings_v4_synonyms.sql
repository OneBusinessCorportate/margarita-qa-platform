-- ---------------------------------------------------------------------------
-- v4: broaden the keyword detector's verb coverage so more real accountant
-- phrasings are auto-recognized (fewer chats stuck on "Предстоящая" purely
-- because the wording didn't match). Mirrors the same additions made to the
-- JS detector in src/lib/mailings-detect.ts:
--   • main_taxes/done:   + подал, отчита(лся), задеклар(ировал)
--   • salary/done:       + отправил/отправила/отправлен(а)/отправили, выслал, переслал
--   • salary/req:        + жду, ожида(ю)
--   • primary_docs/done: + отправил/отправила/отправлен(а)/отправили, выслал, переслал
--   • primary_docs/req:  + жду, ожида(ю)
--   • debts/paid:        + оплачен/оплачена/оплачено (passive), выплатил/выплатила
--   • debts/call:        + созвон(ились)
--
-- The done-side "отправил/отправила/..." forms are spelled out (not the bare
-- "отправ" stem) so they don't also match the req-side imperative "отправьте"
-- — see the same guard in mailings-detect.ts.
--
-- Cycle bounds / rollover logic is unchanged from
-- 20260706_mqa_detect_mailings_v3_cycle28.sql; only the signal regexes move.
-- ---------------------------------------------------------------------------
    ...
    cross join lateral (
      values
        ...
        (case when text ~* '(налог|декларац|ндс|налогов)'
               and text ~* '(отправ|подал|подан|сдан|направил|загрузил|выгрузил|сдала|отправила|отчита|задеклар)'
              then 'main_taxes' end,
         'done'),
        -- salary: done
        (case when text ~* '(зарплат|ведомост|\mзп\M|авансовый\s+отчет|авансов)'
               and text ~* '(получ|пришл|прислал|подпис|сдал|предоставил|скинул|сбросил|прислала|получила|пришла|отправил|отправила|отправлен|отправлена|отправили|выслал|переслал)'
              then 'salary' end,
         'done'),
        -- salary: req (request)
        ...
        (case when text ~* '(первичн|первичк|акт[ыа]?\M|документ|накладн|счет-факт|счёт-факт)'
               and text ~* '(получ|пришл|прислал|сдал|предоставил|скинул|передал|прислала|получила|пришла|отправил|отправила|отправлен|отправлена|отправили|выслал|переслал)'
