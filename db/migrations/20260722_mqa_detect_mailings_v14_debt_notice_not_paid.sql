-- ---------------------------------------------------------------------------
-- mqa_detect_mailings v14 — «Нет долга» больше не срабатывает на УВЕДОМЛЕНИИ о
-- взыскании/неуплате.
--
-- Причина: длинный шаблон «1 июля изменяется порядок взыскания неуплаченных
-- налоговых обязательств … при наличии задолженности … погашен …» содержал и
-- ключ «задолж», и глагол-оплату в одном абзаце → ветка paid давала ложное
-- «Нет долга», хотя это рассылка-НАПОМИНАНИЕ по долгу. Маргарита отмечала такие
-- чаты вручную. Замер за 202607: 8 чатов ложно «Нет долга» только из-за этого
-- шаблона.
--
-- Что делает v14: добавляет guard `debt_notice` (взыскан|неуплач|просроч|порядок
-- взыскан) к RU-ветке paid — уведомление о взыскании больше НЕ считается оплатой.
-- Такой чат ловится мягким debt_soft_ru как debts→'req' («1-й/2-й написал»).
-- Чаты с настоящей оплатой (в тексте есть реальное «долг погашен/оплачен») по-
-- прежнему «Нет долга». Всё остальное байт-в-байт как v13.
-- ---------------------------------------------------------------------------
