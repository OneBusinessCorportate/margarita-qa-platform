-- ---------------------------------------------------------------------------
-- v11: broaden the auto-detection of the STANDARD department mailing templates
-- so the accountant no longer has to mark рассылка by hand (Маргарита:
-- «платформа часто автоматически НЕ подтягивает информацию о рассылке по долгам
-- из чатов … и по зарплате/документам»).
--
-- WHAT changed vs v10 (kept in 1:1 parity with src/lib/mailings-detect.ts):
--   1. DEBTS — the до-5 «оплата бухгалтерских услуг» reminder almost NEVER
--      contains the word «долг» («Оплатите бухгалтерские услуги до X числа»,
--      «Напоминаем о необходимости произвести оплату для продолжения работы»,
--      реквизиты банка + «payment for accountant service»). None of the existing
--      debt rules (all keyed on долг/задолженност) fired → the mailing was never
--      detected. New `svc_pay` marker (imperative / reminder + «услуг» / bank
--      purpose line) → debts:req, WITHOUT a «долг» keyword. It deliberately
--      requires a побудительн./напоминат. form so the welcome-template field
--      «Срок оплаты услуг:» does NOT false-fire.
--   2. SALARY — the до-10 send template («Направляю таблицу по заработным
--      платам, оплаты проставлены в банке», «перечислил зарплату», «выплаты
--      произведены») used send verbs (направ/перечисл/перевёл/простав/произвел)
--      that were absent from the salary done branch → not detected. Added them
--      (and mirrored into neg_send so «зарплату не перечислил» stays not-done),
--      plus 1st-person notify forms (сообща/сообщу/напоминаю/направля).
--   3. REQ — the до-28 document-request template («Просим вас предоставить …
--      инвойс, акт, счёт-фактура … данные для расчёта заработной платы») used
--      «просим»/«предоставьте», which the req verb list (only «прошу»/«просьб»)
--      missed. Added «просим|просят|предоставьт» to the salary & primary_docs
--      req branches.
--   4. TAXES guard — the до-5 service-payment reminder mentions «налоговую
--      оптимизацию»; a `!~* svc_pay` guard keeps it out of the taxes-done branch.
--
-- Idempotent: `create or replace`. Everything else is byte-identical to v10.
