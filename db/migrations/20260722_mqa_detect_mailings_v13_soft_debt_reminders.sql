-- ---------------------------------------------------------------------------
-- mqa_detect_mailings v13 — «мягкое» распознавание рассылок по ДОЛГАМ.
--
-- Запрос QA (Маргарита, 2026-07-21/22): «Платформа не подтягивает автоматически
-- информацию о рассылке по долгам из чатов — приходится отмечать вручную».
-- Примеры: web.telegram.org/a/#-5069704122, #-4909204930, #-5288416775, а также
-- ранее #-4978043895 / #-5288416775 («проблема по-прежнему сохраняется»).
--
-- Причина: рассылка о долге у бухгалтеров часто НЕ содержит слова «долг/
-- задолженность». Это шаблонные напоминания об оплате и о взыскании:
--   • «1 июля изменяется порядок ВЗЫСКАНИЯ НЕУПЛАЧЕННЫХ налоговых обязательств…»
--   • «Сегодня ПОСЛЕДНИЙ ДЕНЬ для оплаты», «оплатить … ДО … срока»
--   • «напоминаем … оплатить», «просим погасить …»
--   • арм.: «գանձ…», «վճարել մինչև …», «չվճար…»
-- Такие сообщения раньше не попадали в категорию debts.
--
-- Что делает v13: ДОБАВЛЯЕТ (не заменяет) два «мягких» сигнала debts→'req'
-- (RU + HY) для этого шаблона. Это может только ДОБАВИТЬ отметку рассылки по
-- долгу («1-й/2-й написал»); статусы «Нет долга»/«позвонил» имеют приоритет и
-- не понижаются, ручные (source='manual') строки по-прежнему не перетираются.
-- Всё остальное (налоги / зарплата / первичка, negation-guard'ы, окно периода,
-- upsert с manual-lock) — байт-в-байт как в live-определении.
--
-- КАНОНИЧЕСКОЕ МЕСТО функции — QA-платформа (repo #1). Здесь та же копия.
-- ---------------------------------------------------------------------------
