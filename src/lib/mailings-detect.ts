...
// v15: расширено на формулировки без слова «услуги» — «оплатить … стоимость
// бухгалтерского ОБСЛУЖИВАНИЯ», «оплата бухгалтерского обслуживания» (Маргарита:
// рассылка по оплате/долгам часто не подтягивается; в чатах B-4611 и др.
// напоминание звучало как «Необходимо оплатить … стоимость бухгалтерского
// обслуживания»). Строго аддитивно к v11.
// NB: `\w` is ASCII-only even under the `u` flag, so Cyrillic/Armenian stem
// CONTINUATIONS use `[\p{L}]` (Unicode letter) — mirrors the SQL `[[:alpha:]]`.
const SVC_PAY_REMINDER =
  /(?:оплатите\s+(?:бухгалтерск|наши\s+услуг|услуг)|произвести\s+оплату|о\s+необходимости\s+(?:произвести\s+)?оплат|напоминаем[^.!?]{0,40}?оплат|напоминаем\s+о\s+(?:необходимости\s+)?(?:произвести\s+)?оплат|просим\s+оплат|оплат[\p{L}]*\s+(?:[\p{L}]+\s+){0,3}?бухгалтерск[\p{L}]*\s+(?:услуг|обслуживани)|стоимост[\p{L}]*\s+бухгалтерск[\p{L}]*\s+обслуживани|payment\s+for\s+account(?:ant|ing)\s+servic|pay\s+for\s+account(?:ing|ant)\s+servic|կատարեք\s+հաշվապահական\s+ծառայ[\p{L}]*\s+վճար|վճարումը\s+կատար|խնդրում\s+ենք\s+վճար[\p{L}]*\s+կատար|հաշվապահական\s+(?:ծառայ|սպասարկ)[\p{L}]*\s+վճար)/iu;
