...
const SVC_PAY_REMINDER =
  /(?:оплатите\s+(?:бухгалтерск|наши\s+услуг|услуг)|произвести\s+оплату|о\s+необходимости\s+(?:произвести\s+)?оплат|напоминаем[^.!?]{0,40}?оплат|напоминаем\s+о\s+(?:необходимости\s+)?(?:произвести\s+)?оплат|просим\s+оплат|оплат\w*\s+(?:\w+\s+){0,3}?бухгалтерск\w*\s+(?:услуг|обслуживани)|стоимост\w*\s+бухгалтерск\w*\s+обслуживани|payment\s+for\s+account(?:ant|ing)\s+servic|pay\s+for\s+account(?:ing|ant)\s+servic|կատարեք\s+հաշվապահական\s+ծառայ\w*\s+վճար|վճարումը\s+կատար|խնդրում\s+ենք\s+վճար\w*\s+կատար|հաշվապահական\s+(?:ծառայ|սպասարկ)\w*\s+վճար)/iu;
