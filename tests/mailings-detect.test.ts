...
// ---------------------------------------------------------------------------
// v16 — Chinese (中文) mailing detection. The firm serves Chinese-speaking
// clients and the standard templates also go out in Chinese; none of these were
// detected before. These lock the real templates the owner pasted plus the
// negation / false-positive guards, in parity with the SQL engine.
// ---------------------------------------------------------------------------
describe("detectAllSignals — Chinese (中文) v16", () => {
  const has = (text: string, cat: string, type: string) =>
    detectAllSignals(text).some((s) => s.category === cat && s.type === type);

  // The owner's real service-payment reminder (down to the bank details).
  const ZH_SERVICE = `提交报表及进行税务优化
请于每月5日前支付会计服务费用，以便我们将该费用计入当期成本，并相应降低税负。
付款信息：
• 账户号码：1930097970708600（AMD）
• 银行：Converse Bank
• 收款方：Business Tech LLC
• 付款用途：会计服务费
• 金额：25,000 AMD
收到付款后，我们将继续为您提供完整的会计服务。谢谢！`;

  // The owner's real document + payroll info request.
  const ZH_REQUEST = `为了确保报表及时、准确地编制，请提供 6 月份的以下信息：
关于开具发票的相关信息（Invoice、服务确认单、发票等）。
进出口相关文件（合同、Invoice、CMR、报关单、商品文件、HS编码、重量等）。
所有银行账户及所有币种的银行流水，从年初至对应月份月底。
工资计算所需的数据。
关于2025年是否持有居留许可（ВНЖ）或工作许可（Work Permit）的信息。`;

  it("service-payment reminder (支付会计服务费用) → debts/req, NOT taxes", () => {
    assert.ok(has(ZH_SERVICE, "debts", "req"), "expected debts/req");
    assert.equal(statusOf("debts", ZH_SERVICE), "1-й написал");
    assert.notEqual(statusOf("main_taxes", ZH_SERVICE), "Отправил",
      "«税务优化/提交报表» must not be misread as a sent tax mailing");
  });

  it("document + payroll info request → primary_docs/req AND salary/req", () => {
    assert.ok(has(ZH_REQUEST, "primary_docs", "req"), "expected primary_docs/req");
    assert.ok(has(ZH_REQUEST, "salary", "req"), "expected salary/req");
    assert.equal(statusOf("primary_docs", ZH_REQUEST), "Запросил 1, не получил");
    assert.equal(statusOf("salary", ZH_REQUEST), "Запросил 1, не получил");
  });

  it("tax submitted (已在银行开具) → main_taxes/Отправил", () => {
    assert.equal(statusOf("main_taxes", "请缴纳报关税款，税单已在银行开具。谢谢"), "Отправил");
  });

  it("salary table sent (工资 + 已发放/通知) → Получил", () => {
    assert.equal(statusOf("salary", "工资表已发放，工资已在银行提交，特此通知。"), "Получил");
  });

  it("no-employees payroll notice (通知 + 工资) → sent = Получил", () => {
    assert.equal(
      statusOf("salary", "温馨提示：本期公司无员工，未进行工资计算。"),
      "Получил",
      "sending the «no salary this period» notice IS a completed mailing"
    );
  });

  it("debt paid (欠款已结清) → Нет долга", () => {
    assert.equal(statusOf("debts", "客户欠款已结清，无欠款。"), "Нет долга");
  });

  it("debt reminder (催款/请支付 欠款) → debts/req", () => {
    assert.ok(has("提醒您尽快支付逾期欠款。", "debts", "req"));
  });

  it("negation: 尚未支付/未提交 is never done/paid", () => {
    assert.ok(!has("工资表尚未提交，暂未收到数据。", "salary", "done"));
    assert.ok(has("工资表尚未提交，暂未收到数据。", "salary", "neg"));
    assert.ok(!has("税款尚未申报。", "main_taxes", "done"));
    assert.ok(has("税款尚未申报。", "main_taxes", "neg"));
  });

  it("false positive guard: Chinese small talk → no categories", () => {
    assert.deepEqual(detectAllSignals("您好！今天过得怎么样？"), []);
  });

  it("false positive guard: «报表»/«税务» alone (no specific keyword) → no tax done", () => {
